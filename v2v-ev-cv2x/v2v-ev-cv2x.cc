...
main (int argc, char *argv[])
{
  ...
  std::string sumo_folder = "scratch/v2v-ev-cv2x/sumo_v2v_map/";
  ...
  std::string csv_name;
  std::string csv_name_cumulative = "ev-cum";
  ...
  if(m_prr_sup)
    {
      if(csv_name_cumulative!="")
      {
        ...
        else
        {
          ...
          csv_cum_ofstream << "num_nodes,t1,t2,current_txpower_dBm,avg_PRR,avg_latency_ms" << std::endl;
        }
