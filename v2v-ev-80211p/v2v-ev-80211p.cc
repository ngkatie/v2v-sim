...
main (int argc, char *argv[])
{
  ...
  /*** 0.a App Options ***/
  std::string sumo_folder = "scratch/v2v-ev-80211p/sumo_v2v_map/";
  std::string mob_trace = "cars.rou.xml";
  std::string sumo_config ="scratch/v2v-ev-80211p/sumo_v2v_map/map.sumo.cfg";
  ...
  std::string csv_name;
  std::string csv_name_cumulative = "ev_cumulative";
  ...
  if(m_prr_sup)
    {
      if(csv_name_cumulative!="")
      {
        ...
        else
        {
          ...
          csv_cum_ofstream << "num_nodes,avg_PRR,avg_latency_ms" << std::endl;
        }

        csv_cum_ofstream << numberOfNodes << "," << prrSup->getAveragePRR () << "," << prrSup->getAverageLatency () << std::endl;
      }
